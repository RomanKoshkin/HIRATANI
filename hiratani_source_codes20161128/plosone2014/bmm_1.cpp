...
class Model {
    public:
        ...
        int SNE = (int)floor(NE*h/tmE + 0.001);
        int SNI = (int)floor(NI*h/tmI + 0.001);
		...
		ParamsStructType getState();
        vector< vector<double> > calc_J(double, double);
        vector<int> rnd_sample(int, int);
        double fd(double, double);

		vector<vector<double>>* retArray();
        // double* convertToArray(vector<vector<double>>& , int, int);
    private:
        double xx;
};
...
Model::ParamsStructType Model::getState(){
	Model::ParamsStructType ret_struct;
...
// double** Model::convertToArray(vector<vector<double>>& vals, int N, int M) {
// 	double** temp;
// 	temp = new double*[N];
// 	for(int i=0; i<N; i++) {
// 		temp[i] = new double[M];
...
vector<vector<double>>* Model::retArray() {
	vector<double> empty_row;
	
	// populate a row
	for (int j=0; j<3; j++) {
		empty_row.push_back(0.1 * (double) j + (double) j);
	}
	// add rows to create a matrix
	for (int i=0; i<3; i++) {
		Ji.push_back(empty_row);
	}
	return &Ji;
}
...
{   
	...
	double* getWeights(Model* m) {
		...
		// const int x = 3;
		// double* arrayOfPointers = new double[x*x]; // ?
		// vector<vector<double>>* arrayOfPointersVec = m->retArray();
		
		for (int i=0; i<x; i++) {
			for (int j=0; j<x; j++) {
				arrayOfPointers[j + x*i] = (double)(*arrayOfPointersVec)[i][j];
				// std::cout << arrayOfPointers[i][j] << std::endl;
			}
		}
	...
	Model::ParamsStructType getState(Model* m) {
		Model::ParamsStructType ret;
		ret = m->getState();
		ret.hh = 1234.01;
        return ret;
		// return m->getState();
