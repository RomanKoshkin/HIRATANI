// Model of excitatory and inhibitory STDP
//
...
double const wLo = 50.0;
double const tppL = 20.0;
double const tpdL = 20.0;
double const ApL = 1.0;
//double const AdL = ApL*tppL/tpdL;
//double const etaL = 0.1*eta*wLo/wEo;//eta*wLo/wEo;
double const wLmin = 0.1;
double const wLmax = 100.0;

//STDP : E-to-I (L) : Hebbian
double const wIo = 25.0;
double const tppI = 20.0;
double const tpdI = 20.0;
double const ApI = 1.0;
//double const AdI = ApI*tppI/tpdI;
//double const etaI = 0.1*eta*wIo/wEo; //0.1*eta*wIo/wLo;//eta*wIo/wLo;
double const wImin = 0.1;
double const wImax = 50.0;
...
//int const dEdmin = (int)floor(0.1/h + 0.01); //0.1ms
//int const dEdmax = (int)floor(0.2/h + 0.01); //0.2ms for comparison dendritic delay is neglected

int const dEmax = dEamax + dEdmax;
int const dlmin = (int)floor(0.2/h + 0.01); //0.2ms
int const dlmax = (int)floor(1.2/h + 0.01); //1.2ms
int const dld = (int)floor(0.2/h + 0.01); //0.2ms

double gtmax = 20.0; //[ms]
int igtmax = (int)floor(gtmax/h);

//correlation
int const kg = 3; //for comparison, kg = 1 is considered
double const ttheta = 2.0; //[ms]
...
void calc(double qA, double qB, double gmL, double gmI, double ALbo, double AIbo, int ik){
	int iqA = (int)floor(qA*100.1);
	int iqB = (int)floor(qB*100.1);
	int igmL = (int)floor(gmL*100.0 + 0.01);
	int igmI = (int)floor(gmI*100.0 + 0.01);
	int iALbo = (int)floor(ALbo*100.0 + 0.01);
	int iAIbo = (int)floor(AIbo*100.0 + 0.01);
	int ittheta = (int)floor( ttheta*100.01 );
	
	ostringstream ossd; 
	ossd << "eistdp_d_a" << iqA << "_b" << iqB << "_l" << igmL << "_i" << igmI << "_al" << iALbo << "_ai" << iAIbo << "_k" << ik << ".txt";
	string fstrd = ossd.str(); ofstream ofsd; ofsd.open( fstrd.c_str() );	
	ostringstream osss; 
	osss << "eistdp_s_a" << iqA << "_b" << iqB << "_l" << igmL << "_i" << igmI << "_al" << iALbo << "_ai" << iAIbo << "_k" << ik << ".txt";
	string fstrs = osss.str(); ofstream ofss; ofss.open( fstrs.c_str() );	
	ofss.precision(10);	
	ostringstream ossr; 
	ossr << "eistdp_r_a" << iqA << "_b" << iqB << "_l" << igmL << "_i" << igmI << "_al" << iALbo << "_ai" << iAIbo << "_k" << ik << ".txt";
	string fstrr = ossr.str(); ofstream ofsr; ofsr.open( fstrr.c_str() );
	ofsr.precision(10);
	ostringstream ossw; 
	ossw << "eistdp_w_a" << iqA << "_b" << iqB << "_l" << igmL << "_i" << igmI << "_al" << iALbo << "_ai" << iAIbo << "_k" << ik << ".txt"; 
	string fstrw = ossw.str(); ofstream ofsw; ofsw.open( fstrw.c_str() );

	double const etaL = ALbo*eta*wLo/wEo;
	double const etaI = AIbo*eta*wIo/wEo;

	double const AdL = gmL*ApL*tppL/tpdL;
	double const AdI = gmI*ApI*tppI/tpdI;
	...
	//Hebbian-lateral
	vector< vector<double> > ydL; //LTD
	vector< vector< deque<double> > > uydL; //LTD-post
	vector< deque< vector<int> > > sydL; //LTD-pre
	vector< vector<double> > ypL; //LTP
	vector< vector< deque<double> > > uypL; //LTP-pre
	vector< deque< vector<int> > > sypL; //LTP-post
	...
	for(int k = 0; k < N; k++){
		...
		for(int j = 0; j < M; j++){
			ydL[k].push_back(0.0);
			uydL[k].push_back(ddeque);
			for(int di = 0; di < dlmax+2; di++) uydL[k][j].push_back(0.0);
		}
		sydL.push_back( iveque );
		for(int di = 0; di < dlmax+2; di++) sydL[k].push_back(ivec);
	}
	for(int j = 0; j < M; j++){
		ypL.push_back( dvec );
		uypL.push_back( ddec );
		for(int k = 0; k < N; k++){
			ypL[j].push_back(0.0);
			uypL[j].push_back(ddeque);
			for(int di = 0; di < dlmax+2; di++) uypL[j][k].push_back(0.0);
		}
		sypL.push_back( iveque );
		for(int di = 0; di < dlmax+2; di++) sypL[j].push_back(ivec);
	}

	//Hebbian-inhibitory
	vector< vector<double> > ydI; //LTD
	vector< vector< deque<double> > > uydI; //LTD-post
	vector< deque< vector<int> > > sydI; //LTD-pre
	vector< vector<double> > ypI; //LTP
	vector< vector< deque<double> > > uypI; //LTP-pre
	vector< deque< vector<int> > > sypI; //LTP-post
	...
	for(int j = 0; j < M; j++){
		...
		for(int k = 0; k < N; k++){
			ydI[j].push_back(0.0);
			uydI[j].push_back(ddeque);
			for(int di = 0; di < dlmax+2; di++) uydI[j][k].push_back(0.0);
		}
		sydI.push_back( iveque );
		for(int di = 0; di < dlmax+2; di++) sydI[j].push_back(ivec);
	}
	for(int k = 0; k < N; k++){
		ypI.push_back( dvec );
		uypI.push_back( ddec );
		for(int j = 0; j < M; j++){
			ypI[k].push_back(0.0);
			uypI[k].push_back(ddeque);
			for(int di = 0; di < dlmax+2; di++) uypI[k][j].push_back(0.0);
		}
		sypI.push_back( iveque );
		for(int di = 0; di < dlmax+2; di++) sypI[k].push_back(ivec);
	}
	...
	for(double t = 0; t < T; t += h){
		...
		for(int j = 0; j < M; j++){ //excitatory update
			...
			if( poisson(rhoE[j]) == 1 ){
				...
				for(int kidx = 0; kidx < wIEoutidx[j].size(); kidx++){
					...
					uypL[j][k][dLs[k][j]] += etaL*(1.0 + ssig*ngn());
					sydL[k][dLs[k][j]].push_back(j);
				}
				for(int kidx = 0; kidx < wEIinidx[j].size(); kidx++){
					int k = wEIinidx[j][kidx];
					uydI[j][k][dld] += etaI*(1.0 + ssig*ngn());
					sypI[k][dld].push_back(j);
				}
				for(int i = 0; i < L; i++){
		...
		for(int k = 0; k < N; k++){ //inhibitory update
			...
			if( poisson(rhoI[k]) == 1 ){
				...
				for(int jidx = 0; jidx < wEIoutidx[k].size(); jidx++){
					...
					uydL[k][j][dld] += etaL*(1.0 + ssig*ngn());
					sypL[j][dld].push_back(k);
				}
				for(int jidx = 0; jidx < wIEinidx[k].size(); jidx++){
					int j = wIEinidx[k][jidx];
					uypI[k][j][dIs[j][k]] += etaI*(1.0 + ssig*ngn());
					sydI[j][dIs[j][k]].push_back(k);
				}
			}
		}
		//weight update
		...
		for(int j = 0; j < M; j++){
			for(int k = 0; k < N; k++){ 			
				ypL[j][k] += uypL[j][k][0]; ypL[j][k] = rk(ypL[j][k],tppL);
				uypL[j][k].pop_front(); uypL[j][k].push_back(0.0);
			}
			for(int kidx = 0; kidx < sypL[j][0].size(); kidx++){
				int k = sypL[j][0][kidx]; 
				wLs[k][j] += ypL[j][k]*ApL;
				if( wLs[k][j] < wLmin ) wLs[k][j] = wLmin;
				if( wLs[k][j] > wLmax ) wLs[k][j] = wLmax;
			}
			sypL[j].pop_front(); sypL[j].push_back(ivec);
		}
		//L-LTD
		for(int k = 0; k < N; k++){ 
			for(int j = 0; j < M; j++){		
				ydL[k][j] += uydL[k][j][0]; ydL[k][j] = rk(ydL[k][j],tpdL);
				uydL[k][j].pop_front(); uydL[k][j].push_back(0.0);
			}
			for(int jidx = 0; jidx < sydL[k][0].size(); jidx++){
				int j = sydL[k][0][jidx]; 
				wLs[k][j] += -ydL[k][j]*AdL;
				if( wLs[k][j] < wLmin ) wLs[k][j] = wLmin;
				if( wLs[k][j] > wLmax ) wLs[k][j] = wLmax;
			}
			sydL[k].pop_front(); sydL[k].push_back(ivec);
		}

		//I-LTP
		for(int k = 0; k < N; k++){
			for(int j = 0; j < M; j++){ 			
				ypI[k][j] += uypI[k][j][0]; ypI[k][j] = rk(ypI[k][j],tppI);
				uypI[k][j].pop_front(); uypI[k][j].push_back(0.0);
			}
			for(int jidx = 0; jidx < sypI[k][0].size(); jidx++){
				int j = sypI[k][0][jidx]; 
				wIs[j][k] += ypI[k][j]*ApI; //hebbian
				if( wIs[j][k] < wImin ) wIs[j][k] = wImin;
				if( wIs[j][k] > wImax ) wIs[j][k] = wImax;
			}
			sypI[k].pop_front(); sypI[k].push_back(ivec);
		}
		//I-LTD
		for(int j = 0; j < M; j++){ 
			for(int k = 0; k < N; k++){		
				ydI[j][k] += uydI[j][k][0]; ydI[j][k] = rk(ydI[j][k],tpdI);
				uydI[j][k].pop_front(); uydI[j][k].push_back(0.0);
			}
			for(int kidx = 0; kidx < sydI[j][0].size(); kidx++){
				int k = sydI[j][0][kidx]; 
				wIs[j][k] += -ydI[j][k]*AdI; // hebbian
				if( wIs[j][k] < wImin ) wIs[j][k] = wImin;
				if( wIs[j][k] > wImax ) wIs[j][k] = wImax;
			}
			sydI[j].pop_front(); sydI[j].push_back(ivec);
		}
...
void simul(double qA, double qB, double gmL, double gmI, double ALbo, double AIbo, int ik){
	calc(qA, qB, gmL, gmI, ALbo, AIbo, ik);
}

int main(int argc, char **argv){
	double qA = 0.0; double qB = 0.0; double gmL = 0; double gmI = 0.0; 
	double ALbo = 0; double AIbo = 0.0; int ik = 0;
	if(argc > 1) qA = atof(argv[1]);
	if(argc > 2) qB = atof(argv[2]);
	if(argc > 3) gmL = atof(argv[3]);
	if(argc > 4) gmI = atof(argv[4]);
	if(argc > 5) ALbo = atof(argv[5]);
	if(argc > 6) AIbo = atof(argv[6]);
	if(argc > 7) ik = atoi(argv[7]);
	srand((unsigned int)time(NULL));
	simul(qA,qB,gmL,gmI,ALbo,AIbo,ik);
	return 0;
}
