...
// sudo /usr/bin/g++ -std=gnu++11 -O3 -dynamiclib -ftree-vectorize -march=native -mavx -g bmm.cpp -o ./bmm.dylib
...
void Model::setParams(Model::Params params){
    std::cout << params.a << std::endl;
    std::cout << params.b << std::endl;
	int x = 0;
}

Model::retStruct Model::getState(){
	Model::retStruct ret_struct;
	ret_struct.ita = ita;
	ret_struct.itb = itb;
	return ret_struct;
}
...
{   
    ...
    void sim(Model* m, int interval) {
		std::cout << interval << std::endl;
        m->sim(interval);
    }

    void setParams(Model* m, Model::Params params) {
        m->setParams(params);
    }

	Model::retStruct getState(Model* m) {
        return m->getState();
    }
