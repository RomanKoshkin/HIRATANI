...
void Model::STF(double t) {
    #if HAGA == 1
    if( ( (int)floor(t/h) )%10 == 0 ){
        for(int j = 0; j < NE; j++){
            F[j] += (0.99 - F[j])/600.0; //usd = 0.99
            D[j] += (1.0 - D[j])/600.0;
...
void Model::updateMembranePot(int i) {
	...
	#if HAGA == 1
	fdstr << t << " " << mean(ys) << " " << endl;
	#else
	fdstr << t << " " << mean(F) << " " << mean(D) << " " << endl;
	#endif
...
double Model::mean(vector<double> v) {
	auto n = v.size(); 
	double average = 0.0;
	if ( n != 0) {
...
void Model::sim(int interval){
	...
	while (interval > 0){
		...
		for(int iidx = 0; iidx < smpld.size(); iidx++){
			...
			// STDP UPDATE IF THE CURRENTLY CHOSEN (i-th) neuron spikes.
		...
		// EVERY 10 ms Becuase STF is slow, we can apply STF updates every 10th step
        // STF(t);
		...
		// EVERY 100s logging, reporting, dumping
