...
// Comments by Roman Koshkin (roman.koshkin@gmail.com)
...
// ****** for debugging, we explicitly instantiate the entire classes   ****** 
...
// template class std::vector< std::deque<int> >;
...
double const T = 1800*1000.0; // simulation time, ms
double const h = 0.01; // time step, ms ??????

int const NE = 2500; // Nexc
int const NI = 500;   // Ninh
int const N = NE + NI; //
...
double const hE = 1.0; // Threshold of update of excitatory neurons
double hI = 1.0;       // Threshold of update of inhibotory neurons

double const IEex = 2.0; // Amplitude of steady external input to excitatory neurons
double const IIex = 0.5; // Amplitude of steady external input to inhibitory neurons
double mex = 0.3;        // mean of external input
double sigex = 0.1;      // variance of external input

// Average intervals of update, ms
double const tmE = 5.0;  //t_Eud EXCITATORY
double const tmI = 2.5;  //t_Iud INHIBITORY
...
double const tpp = 20.0;  // tau_p
double const tpd = 40.0;  // tau_d
double const twnd = 500.0; // STDP window lenght, ms

// Coefficients of STDP
double const Cp = 0.1*JEE; // must be 0.01875 (in the paper)
double const Cd = Cp*tpp/tpd; // must be 0.0075 (in the paper)
double const g = 1.25;        // ??????
...
int NEa = (int)floor(NE*a+0.01); // Exact number of excitatory neurons stimulated externally
int pmax = NE/NEa;
...
void calc(vector< vector<double> > Jo, double alpha, double usd, double JEI, int ita,int itb, int ik){
	...
	// make a file and call it this sting
	ofstream ofsw;
	...
	string fstrd = ossd.str();
	ofstream ofsd;
	ofsd.open( fstrd.c_str() );

	double tauh = itauh*1000.0; 	// decay time of homeostatic plasticity, in ms
	double t1 = 30*1000.0;			// stimulus onset, ms
	double t2 = t1 + ita*1000.0;	// stimulus offset, ms
	double t3 = t2 + 1.0*1000.0;	// one sec after stim offset, ms
	double t4 = t3 + itb*1000.0;	// itb offset
	double t5 = t4 + 100*1000.0;	// itb offset + 100 s, ms
	cout << t1 << " " << t2 << " " << t3 << " " << t4 << " " << t5 << endl;

	vector<int> ptn_inv; // Excitatory neurons are slit into 3 types 0 (500), 1 (500), 2 (1500)
	...
	// we initialize excitatory neurons with values of synaptic efficiency
	vector<double> ys;
	for (int i = 0; i < NE; i++) {
	...
	for(double t = 0; t < T+h; t += h){
		...
		for(int iidx = 0; iidx < smpld.size(); iidx++){
			...
			// if a chosen neuron is on
			if( x[i] == 1 ){  
				// then we decrease synaptic efficiency of that neuron by 10%
				ys[i] -= usd*ys[i]; 
				it = spts.find( i );
				if( it != spts.end() ) {
					spts.erase( it++ );
			...
			// EQ.(4) p.12
			// -threshold of updata + steady exc input * mean/var of external stim input
			...
			// if u > 0 the ith neuron spikes
			if( u > 0 ){
				spts.insert(i);			// save the id of the spiking POSTsynaptic neuron
				...
				// *****************************************************************************************
				/* we loop over all the POSTSYNAPTIC excitatory neurons that synapse on the currently chosen
				excitatory neuron AS IF IT WERE PRESYNAPTIC */
				...
				/* we loop over all the presynaptic neurons that synapse on the current postsynaptic neuron (i) chosen for 
				update at the current time step */

				// we loop over all the presynaptic neurons that synapse on the current postsynaptic neuron
		...
		for(int iidx = 0; iidx < smpld.size(); iidx++){
			...
			if( x[i] == 1 ){
				it = spts.find( i );
				if( it != spts.end() ) {
					spts.erase( it++ ); // crazy optimization, but in fact eq.(5) p.12 (filled circle)
				}
				x[i] = 0;
			}
...
// to complile
// sudo /usr/bin/g++ -std=gnu++11 -O3 -ftree-vectorize -march=native -mavx -g binary_model_my.cpp -o ./binary_model_my