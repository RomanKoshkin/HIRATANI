// Model of auditory learning through STDP
//
...
double const T = 2000.0*1000.0;//2000.0*1000.0; //3000*1000.0; //3000*1000.0;
double const h = 0.05;//0.05;//0.05; //0.05; //0.10[ms] f stands for fast-temporal simulation
...
double const nuXo = 10.0;//2.0;

double const freqmin = 1000.0;
double const freqmax = 9000.0;

double prefsig = 40.0; //30.0; //40.0; //50.0;//25.0;//50.0;
double qotot = 0.05; //0.1; //0.2;//0.4;

double prefreqos[4][2] = {{1046.5,1568.0},
						  {1174.7,1760.0},
             			  {1298.5,1661.2},//{1318.5,1661.2},
             			  {1396.9,1864.7}};

double prefamps[4][2] = {{0.6,0.4},
            			{0.3,0.8},//{0.3,0.8},//{0.3,0.7},
            			{0.5,0.5},//{0.5,0.5},//{0.2,1.0},//{1.0,0.2},//{0.8,0.2},
            			{1.0,0.3}};//{0.7,0.3}};
double prefdoubles[4][4] = {{1.0,0.4,0.2,0.1},
               				{1.0,0.3,0.3,0.2},
               				{1.0,0.1,1.0,0.8},
               				{1.0,0.8,0.1,0.1}};
/*
double afs[4][3] = {{1046.5/128.0,1568.0/128.0,1046.5/64.0},
        {1174.7/128.0,1760.0/128.0,1174.7/64.0},
        {1318.5/128.0,1661.2/128.0,1318.5/64.0},
        {1396.9/128.0,1864.7/128.0,1396.9/64.0}};
*/

double afs[4][5] = {{0.4,5.0,10.0,40.0,88.0},
       				{0.6,6.0,8.0,42.0,86.0},
       				{0.2,4.0,7.5,44.0,84.0},
       				{0.3,7.0,6.0,46.0,82.0}};

/*
double aafs[4][5] = {{0.4,0.4,0.2,0.5,0.5},
        			{0.3,0.5,0.2,0.5,0.5},
        			{0.3,0.3,0.4,0.5,0.5},
       				{0.65,0.1,0.2,0.5,0.5}};
*/

double aafs[4][5] = {{0.30,0.4,0.2,0.5,0.5},
        			{0.25,0.5,0.2,0.5,0.5},//{0.25,0.5,0.2,0.5,0.5},
        			{0.24,0.3,0.4,0.5,0.5},
       				{0.61,0.2,0.2,0.5,0.5}};//{0.62,0.2,0.2,0.5,0.5}};//{0.60,0.2,0.25,0.5,0.5}};//{0.65,0.1,0.2,0.5,0.5}};

double aphases[4][5] = {{1.0,0.25,0.65,0.17,0.01},
           				{3.0,0.12,0.32,0.13,0.02},
           				{7.8,0.55,0.40,0.11,0.03},
           				{4.5,0.22,0.71,0.07,0.05}};

double betalf = 5.0; //3.0; //2.0; //5.0; //3.0; //2.0; //3.0; //5.0;
double Zbetalf = 27.24; //4.881; //2.28; //27.24; //4.881; //2.28; //4.881; //27.24;
...
void calc(double wIo, int kg, double ttheta, int ik){
	int iwIo = (int)floor(wIo + 0.01);
	int ittheta = (int)floor( ttheta*100.01 );
	
	ostringstream ossd; 
	ossd << "auditory_d_i" << iwIo << "_g" << kg << "_t" << ittheta << "_k" << ik << ".txt";
	string fstrd = ossd.str(); ofstream ofsd; ofsd.open( fstrd.c_str() );	
	ostringstream osss; 
	osss << "auditory_s_i" << iwIo << "_g" << kg << "_t" << ittheta << "_k" << ik << ".txt";
	string fstrs = osss.str(); ofstream ofss; ofss.open( fstrs.c_str() );	
	ofss.precision(10);	
	ostringstream ossr; 
	ossr << "auditory_r_i" << iwIo << "_g" << kg << "_t" << ittheta << "_k" << ik << ".txt";
	string fstrr = ossr.str(); ofstream ofsr; ofsr.open( fstrr.c_str() );
	ofsr.precision(10);
	ostringstream ossw; 
	ossw << "auditory_w_i" << iwIo << "_g" << kg << "_t" << ittheta << "_k" << ik << ".txt";
	string fstrw = ossw.str(); ofstream ofsw; ofsw.open( fstrw.c_str() );

	vector<double> freqs;
	for(int i = 0; i < L; i++){
		freqs.push_back( exp( i*(log(freqmax)-log(freqmin))/((double)L) + log(freqmin) ) );
	}

	//atmp += prefamps[q][i]*prefdoubles[q][j]*exp( -(ftmp-(1+j)*prefreqos[q][i])*(ftmp-(1+j)*prefreqos[q][i])/(2.0*prefsig*prefsig) )/sqrt(2.0*pi*prefsig*prefsig)
		...
		for(int i = 0; i < L; i++){
			...
			for(int j1 = 0; j1 < 2; j1++){
				for(int j2 = 0; j2 < 4; j2++){
					Qss[q][i] +=  prefamps[q][j1]*prefdoubles[q][j2]*exp( -(ftmp-(1+j2)*prefreqos[q][j1])*(ftmp-(1+j2)*prefreqos[q][j1])/(2.0*(1+j2)*prefsig*(1+j2)*prefsig) )/sqrt(2.0*pi*(1+j2)*prefsig*(1+j2)*prefsig);
				}
			}
			Zqtmp += Qss[q][i];
		}
		for(int i = 0; i < L; i++){
			Qss[q][i] = qotot*L*Qss[q][i]/Zqtmp;
			if(Qss[q][i] > 10.0) Qss[q][i] = 10.0;
			if(Qss[q][i] < 0.00001) Qss[q][i] = 0.0;
			cout << Qss[q][i] << " ";
		}
		cout << endl;
	}		
	...
	for(int i = 0; i < L; i++){
		double nuctmp = nuXo;
		for(int q = 0; q < p; q++) nuctmp = nuctmp - nuo*Qss[q][i];
		if( nuctmp < 0.0 ) nuctmp = 0.0;
		nucs.push_back(nuctmp);
	}
	...
	vector<int> spt; double atmp;
	for(double t = 0; t < T; t += h){
		for(int q = 0; q < p; q++){
			atmp = 0.0;
        	for(int j1 = 0; j1 < 5; j1++){
            	atmp += aafs[q][j1]*cos( 2.0*pi*afs[q][j1]*(t/1000.0-aphases[q][j1]) );
			}
			rhos[q] = nuo*exp(betalf*atmp)/Zbetalf;
			/*
			if( poisson(rhos[q])==1 ){
		...
		for(int i = 0; i < L; i++){
			nutmp = nucs[i];
			for(int q = 0; q < p; q++) nutmp += Qss[q][i]*rhos[q];
			if( poisson(nutmp) == 1 ){
				spt.push_back( i );
				if( rtof ){
					if( dice() < nucs[i]/nutmp ){
						ofsd << t << " " << i << " " << -1 << endl;
					}else{
						double rtmp = dice();
						int qsourcetmp = 0; double qsourcecum = 0.0;
						for(int q = 0; q < p; q++){
							if( qsourcecum <= rtmp && rtmp < qsourcecum+Qss[q][i]*rhos[q]/(nutmp-nucs[i]) ){
								qsourcetmp = q;
							}
							qsourcecum += Qss[q][i]*rhos[q]/(nutmp-nucs[i]);
						}
						ofsd << t << " " << i << " " << qsourcetmp << endl;
					}
				}	
			}
		}
...
void simul(double wIo, int kg, double ttheta, int ik){
	calc(wIo, kg, ttheta, ik);
}

int main(int argc, char **argv){
	double wIo = 0.0; 
	int kg = 0; double ttheta = 0.0; int ik = 0;
	if(argc > 1) wIo = atof(argv[1]);
	if(argc > 2) kg = atoi(argv[2]);
	if(argc > 3) ttheta = atof(argv[3]);
	if(argc > 4) ik = atoi(argv[4]);
	srand((unsigned int)time(NULL));
	simul(wIo,kg,ttheta,ik);
	return 0;
}
